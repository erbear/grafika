...
LabiryntGen::LabiryntGen(int poziom)
{
	wersja=1;
	this->n=poziom*poziom;
	this->v = poziom;
	this->odwiedzony = new int[n+1];
	for (int i = 0; i<n+1;i++){
		this->odwiedzony[i] = 0;
	}
	this->graniczacy = new int[n+1];
	for (int i = 0; i<n+1;i++){
		this->graniczacy[i] = 0;
	}
}
...
vector <int> LabiryntGen::Generuj()
{
	...
	for (int i =0;i<n;i++)
	{
		...
		if (i ==0)
		{
			...
			z = rand() % n;
			this->odwiedzony[z]=1;//dodaje do odwiedzonego 
		...
		if (dopolaczenia.size()!=0)
		{
			...
			if (dopolaczenia[x] < z){
				this->polaczenia.push_back(dopolaczenia[x]);
				this->polaczenia.push_back(z);
			} else{
				this->polaczenia.push_back(z);
				this->polaczenia.push_back(dopolaczenia[x]);
			}
